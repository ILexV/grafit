# Патчи graphify: C# constructor-DI (#1) + TS `@/`-алиасы (#5)

Два локальных патча экстрактора `graphify` (`graphifyy`, проверено на **v0.8.39**) для проекта
bpm. Оба — кандидаты на один upstream-PR в `github.com/safishamsi/graphify`.

**#1 — C# constructor-DI.** graphify не извлекает рёбра от класса/конструктора к
...
## #5 — TS `@/`-импорты при solution-style tsconfig
**Симптом:** во фронтенде `@/`-импорты (`import {…} from '@/api/auth-token-store'`) не давали
рёбер к реальным файлам/символам — только узлы-заглушки. Бьёт по ВСЕМ `@/`-импортам фронта.

**Корень (не AST):** bpm использует TS **project references** — `frontend/tsconfig.json` содержит
лишь `references: [...]`, а сами `paths: {"@/*": ["./src/*"]}` лежат в `tsconfig.app.json`.
`_read_tsconfig_aliases` (`extract.py:206-276`) идёт по `extends`, но **не по `references`** →
алиасы пусты → `@/` не резолвится.

**Часть C** — в `_read_tsconfig_aliases`, ПОСЛЕ цикла по `paths` (перед `return aliases`),
добавить обход `references` с гейтом «только если у конфига нет своих paths/baseUrl»:
```python
    if not paths and not compiler_options.get("baseUrl"):
        ...
        if isinstance(references, list):
            for ref in references:
                ...
                if not ref_path:
                    continue
                ref_tsconfig = (base_dir / ref_path).resolve()
                if ref_tsconfig.is_dir():
                    ref_tsconfig = ref_tsconfig / "tsconfig.json"
                elif not ref_tsconfig.suffix:
                    ref_tsconfig = ref_tsconfig.with_suffix(".json")
                if ref_tsconfig.exists():
                    aliases.update(_read_tsconfig_aliases(ref_tsconfig, ref_tsconfig.parent, seen))
```
Дальше существующий `_import_js` сам строит `imports_from` (файл→файл) и `imports` (файл→символ).
Проверено: `AuthProvider.tsx --imports_from--> auth-token-store.ts` + символы `getAccessToken/…`.
(Резолв single-file даёт абсолютный id — артефакт; полная пересборка нормализует, рёбра не висят.)

## Переприменение после апгрейда graphifyy
1. Применить части A, B (C# DI) и C (TS references) в `<graphify>/extract.py`.
...
Проверено на bpm 2026-06-16: граф 3978/6946 → 5045/8008 (#1 DI) → **5045/8677** (#5 `@/`-импорты).
