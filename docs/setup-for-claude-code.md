...
- `grafit_search(question, k, project, neighbors, kind)` — семантический поиск по коду, цитаты
  `path:line` + соседи. `kind` = `all|code|tests|docs|prod` (prod = код без тестов/миграций/генерёнки).
