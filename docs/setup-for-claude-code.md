...
- `grafit_dupes(kind, threshold, limit, rerank)` — глобальный скан копипаста по проекту:
  кластеры near-duplicate символов (`kind=prod` по умолчанию). Для аудита «где у нас дубли».
